...
from adet.utils.misc import bezier_to_polygon, sort_points
    ...
    for (img_dict, anno_dict_list) in imgs_anns:
        ...
        for anno in anno_dict_list:
            ...
            if bezierpts:  # list[float]
                ...
                # obj["sem_seg"] = bezierpts

            polypts = anno.get("polys", None)
            if polypts:
                obj["polygons"] = polypts

            # used for clip matching gt, semantic seg
            if polypts: # testr poly format text box
                obj["sem_seg"] = polypts
            else: # abcnet format text box
                polg = bezier_to_polygon(bezierpts,num_sample=20)
                # polg = sort_points(polg)
                obj["sem_seg"] = polg

            # used for clip matching gt
            # [x1,y1,...,xn,yn]
            # obj["sem_seg"] = load_poly_box(anno, dataset_name)
