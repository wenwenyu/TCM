...
class TextEvaluator():
    ...
    def __init__(self, dataset_name, cfg, distributed, output_dir=None):
        ...
        if "totaltext" in dataset_name:
            self._text_eval_gt_path = f"{cfg.DATA_ROOT}/evaluation/gt_totaltext.zip"
            self._word_spotting = True
            self.dataset_name = "totaltext"
        elif "ctw1500" in dataset_name:
            self._text_eval_gt_path = f"{cfg.DATA_ROOT}/evaluation/gt_ctw1500.zip"
            self._word_spotting = False
            self.dataset_name = "ctw1500"
        elif "icdar2015" in dataset_name:
            self._text_eval_gt_path = f"{cfg.DATA_ROOT}/evaluation/gt_icdar2015_abc.zip"
            self._word_spotting = False
            self.dataset_name = "icdar2015"
        elif "custom" in dataset_name:
            self._text_eval_gt_path = f"{cfg.DATA_ROOT}/evaluation/gt_custom.zip"
            self._word_spotting = False
    ...
    def to_eval_format(self, file_path, temp_dir="temp_det_results", cf_th=0.5):
        ...
        with open(file_path, 'r') as f:
            ...
            tmp_det_file = f'temp_all_det_cors_{time_id}.txt'
            with open(tmp_det_file, 'w') as f2:
        ...
        dirn = temp_dir
        lsc = [cf_th] 
        fres = open(tmp_det_file, 'r').readlines()
        for isc in lsc:	
    ...
    def sort_detection(self, temp_dir):
        ...
        if "totaltext" in self.dataset_name:
            if not self.lexicon_type == None:
                lexicon_path = f'{self.data_root}/totaltext/weak_voc_new.txt'
                lexicon_fid=open(lexicon_path, 'r')
                pair_list = open(f'{self.data_root}/totaltext/weak_voc_pair_list.txt', 'r')
                pairs = dict()
            ...
            if not self.lexicon_type == None:
                lexicon_path = f'{self.data_root}/CTW1500/weak_voc_new.txt'
                lexicon_fid=open(lexicon_path, 'r')
                pair_list = open(f'{self.data_root}/CTW1500/weak_voc_pair_list.txt', 'r')
                pairs = dict()
                lexicon_fid=open(lexicon_path, 'r')
                lexicon=[]
                for line in lexicon_fid.readlines():
            ...
            if self.lexicon_type==1: 
                # generic lexicon
                lexicon_path = f'{self.data_root}/icdar2015/GenericVocabulary_new.txt'
                lexicon_fid=open(lexicon_path, 'r')
                pair_list = open(f'{self.data_root}/icdar2015/GenericVocabulary_pair_list.txt', 'r')
                pairs = dict()
                for line in pair_list.readlines():
                    line=line.strip()
                    word = line.split(' ')[0].upper()
            ...
            if self.lexicon_type==2:
                # weak lexicon
                lexicon_path = f'{self.data_root}/icdar2015/ch4_test_vocabulary_new.txt'
                lexicon_fid=open(lexicon_path, 'r')
                pair_list = open(f'{self.data_root}/icdar2015/ch4_test_vocabulary_pair_list.txt', 'r')
                pairs = dict()
                for line in pair_list.readlines():
                    line=line.strip()
                    word = line.split(' ')[0].upper()
        ...
        for i in files:
            if "icdar2015" in self.dataset_name:
                ...
                if self.lexicon_type==3:
                    lexicon_path = f'{self.data_root}/icdar2015/new_strong_lexicon/new_voc_img_' + str(int(i.split('/')[-1].split('.')[0])) + '.txt'
                    lexicon_fid=open(lexicon_path, 'r')
                    pair_list = open(f'{self.data_root}/icdar2015/new_strong_lexicon/pair_voc_img_' + str(int(i.split('/')[-1].split('.')[0])) + '.txt')
                    pairs = dict()
                    for line in pair_list.readlines():
                        line=line.strip()
                        word = line.split(' ')[0].upper()
        ...
        if "icdar2015" in self.dataset_name:
            time_id = datetime.now().strftime(r'%m%d_%H%M_%S%f')
            det_zip_file_name = f'det_{time_id}.zip'
            det_full_zip_file_name = f'det_full_{time_id}.zip'
            os.system('zip -r -q -j '+ det_zip_file_name +' '+output_file+'/*')
            os.system('zip -r -q -j '+ det_full_zip_file_name +' '+output_file_full+'/*')
            shutil.rmtree(origin_file)
            shutil.rmtree(output_file)
            shutil.rmtree(output_file_full)
            return det_zip_file_name, det_full_zip_file_name
        else:
            ...
            zipf = zipfile.ZipFile(f'../{det_zip_file_name}', 'w', zipfile.ZIP_DEFLATED)
            zipdir('./', zipf)
            zipf.close()
            os.chdir("../")

            os.chdir(output_file_full)
            zipf_full = zipfile.ZipFile(f'../{det_full_zip_file_name}', 'w', zipfile.ZIP_DEFLATED)
            zipdir('./', zipf_full)
            zipf_full.close()
            os.chdir("../")
            # clean temp files
    ...
    def evaluate(self):
        ...
        temp_dir = f"temp_det_results_{time_id}/"
