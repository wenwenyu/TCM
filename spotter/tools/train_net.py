...
import weakref
from detectron2.engine import TrainerBase, create_ddp_model, AMPTrainer, SimpleTrainer
...
from adet.data.dataset_mapper import DatasetMapperWithBasis, TryMapDataset
from adet.config import get_cfg
from adet.checkpoint import AdetCheckpointer
from adet.evaluation import TextEvaluator, TESTRTextEvaluator
from adet.data.builtin import register_all_coco

# from adet.config.clip_config import add_clip_config
# from clip_config import add_clip_config
# import clip_config
from adet.projects.testr_clip import add_clip_config
from adet.projects.testr_clip import Ranger


@configurable(from_config=_train_loader_from_config)
def build_try_detection_train_loader(
        dataset,
        *,
        mapper,
        sampler=None,
    ...
    if mapper is not None:
        ...
        dataset = TryMapDataset(dataset, mapper)
...
class Trainer(DefaultTrainer, TrainerBase):
    """
    ...
    def build_train_loader(cls, cfg):
        ...
        return build_try_detection_train_loader(cfg, mapper=mapper)
    ...
    def build_evaluator(cls, cfg, dataset_name, output_folder=None):
        ...
        if evaluator_type == "text":
            if cfg.EVALUATOR_CLASS_TYPE == 'testr_text':
                return TESTRTextEvaluator(dataset_name, cfg, True, output_folder)
    ...
    def build_optimizer(cls, cfg, model):
        ...
        for key, value in model.named_parameters(recurse=True):
            ...
            elif match_name_keywords(key, [cfg.SOLVER.LR_CLIP_TEXT_ENCODER_NAMES]):
                lr = cfg.SOLVER.BASE_LR * 0.0
        ...
        if optimizer_type == "SGD":
            ...
            optimizer = maybe_add_full_model_gradient_clipping(Ranger)(
                params, cfg.SOLVER.BASE_LR,
                 alpha=0.5, k=6, N_sma_threshhold=5,           # Ranger options
                 betas=(.95, 0.999), eps=1e-5, weight_decay=cfg.SOLVER.WEIGHT_DECAY,  # Adam options
...
def setup(args):
    ...
    """
    cfg = get_cfg()
    # clip_config.add_clip_config(cfg)
    add_clip_config(cfg)
