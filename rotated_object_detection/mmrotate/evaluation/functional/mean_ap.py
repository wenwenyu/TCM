...
def match_result(det_results, annotations):
    bs = len(det_results)
    from scipy.optimize import linear_sum_assignment
    iou_calculator = TASK_UTILS.build(dict(type='RBboxOverlaps2D'))
    result = []
    for i in range(bs):
        det_result = det_results[i]
        gt_bboxes = annotations[i]['bboxes']
        gt_labels = annotations[i]['labels']
        pred_bboxes = np.concatenate(det_result)
        cost = - iou_calculator(torch.from_numpy(pred_bboxes), torch.from_numpy(gt_bboxes))
        matched_row_inds, matched_col_inds = linear_sum_assignment(cost)
        preds = pred_bboxes[matched_row_inds]
        # preds[..., -1] = gt_bboxes[matched_col_inds][..., -1]
        det_labels = gt_labels[matched_col_inds]
        result.append([preds[det_labels == j, :] for j in range(15)])
    return result
...
def eval_rbbox_map(det_results,
    ...
    # change the predicted cls scores to the GT cls scores
    # det_results = match_result(det_results, annotations) 
