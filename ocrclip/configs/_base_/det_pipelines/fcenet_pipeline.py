...
train_pipeline_clip_icdar2015 = [
    ...
    dict(type='Pad', size_divisor=32),
    dict(
        type='FCENetCLIPTargets',
        pix_text_match_gt_size_divisors=32, # stage3:32, 2:16, 1:8
        fourier_degree=5,
        level_proportion_range=leval_prop_range_icdar2015),
    dict(
        type='CustomFormatBundle',
        keys=['p3_maps', 'p4_maps', 'p5_maps', 'matching_maps'],
        visualize=dict(flag=False, boundary_key=None)),
    dict(type='Collect', keys=['img', 'p3_maps', 'p4_maps', 'p5_maps', 'matching_maps'])
]
...
img_scale_ctw1500_736_1280 = (1280, 736)
test_pipeline_ctw1500_736_1280 = [
    dict(type='LoadImageFromFile', color_type='color_ignore_orientation'),
    dict(
        type='MultiScaleFlipAug',
        img_scale=img_scale_ctw1500_736_1280,
        flip=False,
        transforms=[
            dict(type='Resize', img_scale=(1280, 800), keep_ratio=True),
            dict(type='Normalize', **img_norm_cfg),
...
leval_prop_range_tt = ((0, 0.25), (0.2, 0.65), (0.55, 1.0))
train_pipeline_clip_tt = [
    dict(type='LoadImageFromFile', color_type='color_ignore_orientation'),
    dict(
        type='LoadTextAnnotations',
        with_bbox=True,
    ...
    dict(
        ...
        pix_text_match_gt_size_divisors=32, # stage3:32, 2:16, 1:8
        fourier_degree=5,
        level_proportion_range=leval_prop_range_tt),
    dict(
        type='CustomFormatBundle',
        keys=['p3_maps', 'p4_maps', 'p5_maps', 'matching_maps'],
        visualize=dict(flag=False, boundary_key=None)),
...
img_scale_tt = (1280, 960)
test_pipeline_tt = [
    dict(type='LoadImageFromFile', color_type='color_ignore_orientation'),
    dict(
        type='MultiScaleFlipAug',
        img_scale=img_scale_tt,
        flip=False,
        transforms=[
            dict(type='Resize', img_scale=(1280, 800), keep_ratio=True),
            dict(type='Normalize', **img_norm_cfg),
