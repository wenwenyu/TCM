...
def load_img_info(files, dataset):
    ...
    if dataset == 'icdar2017' or dataset == 'mlt2019':
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Convert Icdar2015 or Icdar2017 mlt2019 annotations to COCO format'
    )
    parser.add_argument('icdar_path', help='icdar root path')
    parser.add_argument('-o', '--out-dir', help='output path')
    parser.add_argument(
        '-d', '--dataset', required=True, help='icdar2017 or icdar2015 or mlt2019')
    parser.add_argument(
        '--split-list',
        nargs='+',
...
'''
python tools/data/textdet/icdar_converter.py /apdcephfs/share_887471/common/ocr_benchmark/mmocr_det_data/mlt2019 --dataset mlt2019 --split-list training --nproc 4

python tools/data/textdet/icdar_converter.py /home/wwyu/dataset/mmocr_det_data/icdar2017 --dataset icdar2017 --split-list test --nproc 8
'''
def main():
    args = parse_args()
    ...
    for split, json_name in set_name.items():
        ...
        with mmcv.Timer(print_tmpl='It takes {}s to convert icdar annotation'):
            files = collect_files(
                osp.join(img_dir, split), osp.join(gt_dir, split), args.dataset)
