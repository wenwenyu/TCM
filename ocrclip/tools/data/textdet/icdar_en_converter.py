...
def is_english(gt_file, dataset):
    ...
    for line in gt_list:
        ...
        line = line.strip()
        strs = line.split(',')
        if (dataset == 'icdar2017' and strs[8] != 'English'):
            is_english = False
    return is_english
...
def parse_args():
    ...
    parser.add_argument(
        '--split-list',
        nargs='+',
        help='a list of splits. e.g., "--split-list training val test"')
...
'''
python tools/data/textdet/icdar_en_converter.py /apdcephfs/share_887471/common/ocr_benchmark/mmocr_det_data/icdar2017 --dataset icdar2017 --split-list training val --nproc 4
'''
def main():
    args = parse_args()
    ...
    for split in args.split_list:
        set_name.update({split: 'instances_' + split + '_en.json'})
